<script lang="ts">
	...
	import type { getConcludedYearDetail } from '$lib/server/trek-service';
	type PageData = Awaited<ReturnType<typeof getConcludedYearDetail>>;
...
<svelte:head>
	<title
		>{data.round.year ?? data.selections[0]?.albumName ?? 'Album round'} · {data
			.trek.name} · MTrek</title
	>
</svelte:head>
	...
	<div class="contents">
		...
		<div class="trek-hero">
			<div class="trek-title">
				<p class="eyebrow">
					{data.trek.type === 'curated' ? 'Concluded album' : 'Concluded year'}
				</p>
				<h1>
					{data.round.year ?? data.selections[0]?.albumName ?? 'Album round'}
				</h1>
				<div class="meta-row">
		...
		{#if data.selections.length === 0}
			<p class="empty alert">No albums were recorded for this round.</p>
		{:else}
